...
const DailyCalendarView = ({ entries, startDate, numDays }) => {
  ...
  const visibleData = useMemo(
    () => entries.filter((d) => !new Set(state.excludedCategoryIds).has(d.category?.id)),
  ...
  useEffect(() => {
    ...
    const parsed = filtered.map((d) => {
      ...
      const fullDate = new Date(ts.getFullYear(), ts.getMonth(), ts.getDate(), ts.getHours(), ts.getMinutes(), ts.getSeconds());
    ...
    pointsByClusterY.forEach((pointsAtY) => {
      const pointsByDay = d3.group(pointsAtY, (d) => {
        const f = d.fullDate;
        return new Date(f.getFullYear(), f.getMonth(), f.getDate(), 12).getTime();
      });
  ...
  const handleUpdatePointCategory = (point, newCategoryId) => {
    ...
    setClusteredData((prev) => prev.map((p) => (p.id === point.id ? updatedPoint : p)));
  };
    ...
    <div ref={containerRef} className="w-full h-full flex-grow relative">
      {dimensions.width > 0 && dimensions.height > 0 && (
        <svg ref={svgRef} width={dimensions.width} height={dimensions.height}>
          ...
          <g>
            {d3.timeHour.range(new Date(1970, 0, 1, 6), new Date(1970, 0, 2, 0), 6).map((time, i) => (
              <line
                key={i}
                x1={margin.left * 1.2}
                y1={yScale(time)}
          ...
          <g>
            {d3.timeDay.every(1).range(startDate, endDate).map((date, i) => {
      ...
      <div className="flex justify-center py-2 bg-black bg-opacity-80" style={{ height: dateBarHeight }}>
        <h3 className="text-md text-white font-semibold text-center cursor-pointer" style={{ fontFamily: "Noto Sans JP" }}>
          {"Selected Period: "}
          {startDate
            ? `${d3.timeFormat("%b %d, %Y")(startDate)}${numDays > 1 ? ` - ${d3.timeFormat("%b %d, %Y")(endDate)}` : ""}`
            : "Select Date"}
        </h3>
      </div>
    </div>
